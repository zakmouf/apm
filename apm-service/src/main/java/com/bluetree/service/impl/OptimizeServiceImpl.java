...
import com.bluetree.service.OptimizeService;
...
public class OptimizeServiceImpl extends ServiceSupport implements
	OptimizeService {
    ...
    private void ValidateIndicePrices(OptimizeConfig config,
	    List<Price> indicePrices) throws OptimizeException {
    ...
    private void validateStocks(OptimizeConfig config,
	    List<Stock> indiceStocks, List<Stock> previewStocks)
    ...
    @Override
    public OptimizeResult process(OptimizeConfig config)
	    throws OptimizeException {
	...
	int successPortfolio = 0;
	int unsuccessPortfolio = 0;
	int maxUnsuccessPortfolio = config.getMaxUnsuccessPortfolio();
	Double decisionRatioLimit = config.getStartDecisionRatio();

	logger.debug(msg(
		"found [{0,number,0}] of [{1,number,0}] success with ratio [{2,number,0%}]",
		successPortfolio, config.getSuccessPortfolio(),
		decisionRatioLimit));

	while (successPortfolio < config.getSuccessPortfolio()) {
	    ...
	    CombinedMeasure measure = measureHolder.getCombinedMeasure();

	    boolean isSuccess = true;

	    Double decisionRatio = measure.getRegressionMeasure()
		    .getDecisionRatio();
	    isSuccess = isSuccess && decisionRatio >= decisionRatioLimit;

	    Double beta = measure.getRegressionMeasure().getBeta();
	    isSuccess = isSuccess && beta >= config.getMinBeta();
	    isSuccess = isSuccess && beta <= config.getMaxBeta();

	    Double alpha = measure.getRegressionMeasure().getAlpha();
	    isSuccess = isSuccess && alpha >= config.getMinAlpha();
	    isSuccess = isSuccess && alpha <= config.getMaxAlpha();

	    if (isSuccess) {
		successPortfolio++;
		unsuccessPortfolio = 0;
		logger.debug(msg(
			"found [{0,number,0}] of [{1,number,0}] success with ratio [{2,number,0.0%}]",
			successPortfolio, config.getSuccessPortfolio(),
			decisionRatioLimit));
		logger.debug(msg(
			"measure beta [{0,number,0.00}] alpha [{1,number,0.0%}] ratio [{2,number,0.0%}]",
			beta, alpha, decisionRatio));
	    } else {
		unsuccessPortfolio++;
		if (unsuccessPortfolio >= maxUnsuccessPortfolio) {
		    decisionRatioLimit = decisionRatioLimit - 0.01;
		    unsuccessPortfolio = 0;
		    logger.debug(msg("decrease ratio to [{0,number,0%}]",
			    decisionRatioLimit));
		}
